...
class Main(TableScriptToHtml):
    ...
    def docHeader(self, header):
        ...
        if self.parameter('anno'):
            row.cell("""<center><div style='font-size:14pt;'><strong>Estratto/Statement <br>{cliente}</strong></div>
                    <div style='font-size:10pt;'>{anno}</div></center>::HTML""".format(cliente=self.field('@fornitore_id.rag_sociale'),anno=self.parameter('anno')))
        elif self.parameter('dal'):
            row.cell("""<center><div style='font-size:12pt;'><strong>Estratto/Statement <br>{cliente}</strong></div>
                    <div style='font-size:10pt;'>from {dal} to {al}</div></center>::HTML""".format(cliente=self.field('@fornitore_id.rag_sociale'),
            ...
            row.cell("""<center><div style='font-size:14pt;'><strong>Estratto/Statement</strong></div>
                    <div style='font-size:12pt;'><strong>{cliente}</strong></div></center>::HTML""".format(
                                cliente=self.field('@fornitore_id.rag_sociale')))
    ...
    def gridStruct(self,struct):
        ...
        r = struct.view().rows()
        r.fieldcell('@fornitore_id.rag_sociale', mm_width=62, subtotal='Totale documento {breaker_value}',subtotal_order_by='@fornitore_id.rag_sociale,$data,$doc_n')
    ...
    def gridQueryParameters(self):
        ...
        return dict(condition=' AND '.join(condition), condition_anno=self.parameter('anno'), 
                    ...
                    condition_balance=balance, order_by='@fornitore_id.rag_sociale ASC')#,relation='@forn_fatt')
